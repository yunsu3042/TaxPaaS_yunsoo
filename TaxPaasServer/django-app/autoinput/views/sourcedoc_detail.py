...
from autoinput.serializers import SourceDocSerializer2, SourceDocSerializer3
from member.models import TaxPayerProfile
from rest_framework.response import Response
from rest_framework import status

__all__ = ('SourceDocDetailView', 'SourceDocDetailView2', 'SourceDocDetailView3')


class SourceDocDetailView(generics.RetrieveAPIView):
    queryset = SourceDoc.objects.all()
    serializer_class = SourceDocSerializer2
...
class SourceDocDetailView2(generics.RetrieveAPIView):
    queryset = SourceDoc.objects.all()
    serializer_class = SourceDocSerializer2
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def get_fit_source_doc(self, request, *args, **kwargs):
        ...
        request.data._mutable = True
        request.data['tax_payer'] = tax_payer.pk
        source_docs = tax_payer.sourcedoc_set.all()
        if source_docs.exists():
            return source_docs
        else:
            return None
...
class SourceDocDetailView3(generics.RetrieveAPIView):
    queryset = SourceDoc.objects.all()
    serializer_class = SourceDocSerializer3
    permission_classes = (permissions.IsAuthenticated,)
