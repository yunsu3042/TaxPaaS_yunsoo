...
from autoinput.models import Ten99DIV
from autoinput.serializers import SourceDocSerializer
from autoinput.serializers import Ten99DIVSerializer, Ten99DIVSerializer2

__all__ = ('DivCreateView2', )


## 유저 생성할 때 tax_payer설정해 줘야함
class DivCreateView(generics.CreateAPIView):
    queryset = Ten99DIV.objects.all()
    serializer_class = Ten99DIVSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        print(request.data)
        return self.div_create(request, *args, **kwargs)

    def div_create(self, request, *args, **kwargs):
        source_doc = request.data['source_doc']
        order = request.data['order']
        if source_doc:
            request.data['source_doc'] = request.data['source_doc'].pk
            if Ten99DIV.objects.filter(source_doc=source_doc.pk).filter(order=order).exists():
...
class DivCreateView2(generics.CreateAPIView):
    queryset = Ten99DIV.objects.all()
    serializer_class = Ten99DIVSerializer2
    permission_classes = (permissions.IsAuthenticated, )
    ...
    def div_create(self, request, *args, **kwargs):
        ...
        if source_doc:
            request.data['source_doc'] = request.data['source_doc'].pk
            if Ten99DIV.objects.filter(source_doc=source_doc.pk).filter(order=order).exists():
                return Response({"errors": "이미 div가 존재합니다 생성하고 싶다면 지우고 오세요"},
                                status=status.HTTP_400_BAD_REQUEST)
            else:
                return self.create(request, *args, **kwargs)
        else:
