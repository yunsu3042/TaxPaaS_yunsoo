...
from autoinput.serializers import Ten99INTSerializer

__all__ = ('IntCreateView2', )


## 유저 생성할 때 tax_payer설정해 줘야함
class IntCreateView(generics.CreateAPIView):
    queryset = Ten99INT.objects.all()
    serializer_class = Ten99INTSerializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        print(request.data)
        return self.int_create(request, *args, **kwargs)

    def int_create(self, request, *args, **kwargs):
        source_doc = request.data['source_doc']
        order = request.data['order']
        if source_doc:
            request.data['source_doc'] = request.data['source_doc'].pk
            if Ten99INT.objects.filter(source_doc=source_doc.pk).filter(order=order).exists():
...
class IntCreateView2(generics.CreateAPIView):
    queryset = Ten99INT.objects.all()
    serializer_class = Ten99INTSerializer
    permission_classes = (permissions.IsAuthenticated, )
    ...
    def int_create(self, request, *args, **kwargs):
        ...
        if source_doc:
            request.data['source_doc'] = request.data['source_doc'].pk
            if Ten99INT.objects.filter(source_doc=source_doc.pk).filter(order=order).exists():
                return Response({"errors": "이미 int가 존재합니다 생성하고 싶다면 지우고 오세요"},
                                status=status.HTTP_400_BAD_REQUEST)
            else:
                return self.create(request, *args, **kwargs)
        else:
