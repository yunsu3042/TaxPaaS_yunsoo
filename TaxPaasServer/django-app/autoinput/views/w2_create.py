...
class W2CreateView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        request.data['source_doc'] = self.get_source_doc(request, *args, **kwargs).pk
        print(request.data)
    ...
    def w2_create(self, request, *args, **kwargs):
        ...
        if source_doc:
            if W2.objects.filter(source_doc=source_doc).filter(order=order).exists():
